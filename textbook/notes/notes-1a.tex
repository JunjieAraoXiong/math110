...
%------------------------------------------------------------------------------
% THEOREM ENVIRONMENTS (numbered to match textbook: 1.1, 1.2, etc.)
%------------------------------------------------------------------------------
...
\textbf{1.16 Example:} In the statement $x + 0 = x$ for all $x \in \F^n$, the $0$ must be the vector $(0, \ldots, 0)$ because we can only add vectors to vectors.
...
\textbf{Note:} All scalar multiples of $x$ lie on a single line through the origin. This ``direction preservation'' is fundamental to linear maps (Chapter 3).
...
\begin{enumerate}
    \item \textbf{Complex numbers} $\C = \{a + bi : a, b \in \R\}$ with $i^2 = -1$.
    \item \textbf{Arithmetic:} $(a+bi)(c+di) = (ac-bd) + (ad+bc)i$.
    \item \textbf{Field properties} (1.3): commutativity, associativity, identities, inverses, distributivity.
    \item \textbf{$\F$ notation:} $\F$ denotes either $\R$ or $\C$.
    \item \textbf{$\F^n$} is the set of all $n$-tuples $(x_1, \ldots, x_n)$ with $x_j \in \F$.
    \item \textbf{Addition} is coordinate-wise.
    \item \textbf{Scalar multiplication:} $\lambda(x_1, \ldots, x_n) = (\lambda x_1, \ldots, \lambda x_n)$.
    \item \textbf{Zero vector} $0 = (0, \ldots, 0)$ is the additive identity.
\end{enumerate}
