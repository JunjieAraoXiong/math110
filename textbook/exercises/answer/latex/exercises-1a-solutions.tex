...
\begin{solutionbox}
\textbf{Solution:} Let $\alpha = a + bi$ and $\beta = c + di$ where $a, b, c, d \in \R$.
...
\begin{solutionbox}
\textbf{Solution:} Let $\lambda = a + bi$, $\alpha = c + di$, and $\beta = e + fi$ where $a, b, c, d, e, f \in \R$.

Computing the left-hand side:
\begin{align*}
    \lambda(\alpha + \beta) &= (a + bi)\bigl((c + e) + (d + f)i\bigr) \\
    &= a(c + e) - b(d + f) + \bigl(a(d + f) + b(c + e)\bigr)i \\
    &= (ac + ae - bd - bf) + (ad + af + bc + be)i.
\end{align*}

Computing the right-hand side:
\begin{align*}
    \lambda\alpha &= (a + bi)(c + di) = (ac - bd) + (ad + bc)i \\
    \lambda\beta &= (a + bi)(e + fi) = (ae - bf) + (af + be)i
\end{align*}

Adding:
\begin{align*}
    \lambda\alpha + \lambda\beta &= \bigl((ac - bd) + (ae - bf)\bigr) + \bigl((ad + bc) + (af + be)\bigr)i \\
    &= (ac + ae - bd - bf) + (ad + af + bc + be)i.
\end{align*}

Comparing, LHS $=$ RHS. Therefore $\lambda(\alpha + \beta) = \lambda\alpha + \lambda\beta$. \hfill $\square$
\end{solutionbox}

\bigskip

%==============================================================================
% EXISTENCE AND UNIQUENESS (5-6)
%==============================================================================
...
\begin{solutionbox}
\textbf{Solution:} Let $\alpha = a + bi$ where $a, b \in \R$.
...
\begin{solutionbox}
\textbf{Solution:} Isolate $2x$ by subtracting $(4, -3, 1, 7)$ from both sides:
\[
...
\begin{solutionbox}
\textbf{Solution:} If such a $\lambda$ exists, then each component equation must hold simultaneously.
...
\begin{solutionbox}
\textbf{Solution:} Let $x = (x_1, \ldots, x_n)$, $y = (y_1, \ldots, y_n)$, and $z = (z_1, \ldots, z_n)$ where $x_j, y_j, z_j \in \F$.
...
\begin{solutionbox}
\textbf{Solution:} Let $x = (x_1, \ldots, x_n)$ where $x_j \in \F$.
...
\begin{solutionbox}
\textbf{Solution:} Let $x = (x_1, \ldots, x_n)$ and $y = (y_1, \ldots, y_n)$ where $x_j, y_j \in \F$.
